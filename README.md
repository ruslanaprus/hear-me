...
| **Enable file-based caching** | `true` | When enabled, runtime playback audio can be stored in `public://tts/`. When disabled, click-triggered playback is returned directly and is not persisted. |
...
Synthesised runtime files are stored at `public://tts/<hash>.<ext>` (typically `sites/default/files/tts/`). The hash includes the request source, normalized text hash, language, provider, extension, and provider configuration hash, so provider setting changes do not reuse stale audio.
