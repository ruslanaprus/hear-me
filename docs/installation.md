...
| **Queue source fields** | Title plus Body when available until saved | Per-content-type source text for queued audio. Select whether to include the node title and which supported stored text fields to concatenate. The pre-release candidate offers plain text, long text, formatted text, and text with summary; D10.4 blocks release until formatted values pass through Drupal text filters. At least one source is required for every enabled queue bundle. |
