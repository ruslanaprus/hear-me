...
| **Queue source fields** | Title plus Body when available until saved | Per-content-type source text for queued audio. Select whether to include the node title and which supported stored text fields to concatenate. Supported field types are plain text, long text, formatted text, and text with summary. At least one source is required for every enabled queue bundle. |
...
- Provenance-backed generated audio is detached when a node becomes unpublished or otherwise fails the queue eligibility check. Shared generated Media/File entities are deleted only after their last current entity reference is removed. Historical revisions can retain the old target ID; regenerate audio after reverting such a revision.
...
The confirmed admin action uses Drupal Batch API and only creates queue jobs. Cron or queue workers perform the actual TTS synthesis later. Each synthesis queue item contains only a node ID, content hash, and opaque generation token. An attempt-specific internal reservation is refreshed around the transactional node attachment so a late worker cannot commit or complete a newer attempt. The worker reloads current text and skips stale jobs before synthesis, then validates a fresh node and hash while attaching the Media. Confirmed provider failures are delayed and retried up to three times before the job is discarded; temporary provider-discovery, persistent-storage, queue-state, or Media handoff failures are delayed without consuming that budget. Provider logs retain synthesis failure details.
...
With Drush 13.7 or later, the same backfill is available from the command line:
...
Uninstall never deletes persistent Media or File entities. During normal content updates, HearMe retracts only provenance-backed generated audio from nodes that become ineligible and deletes it only after no current entity references remain. Historical revisions can retain an old target ID after generated audio is retracted. Manually selected or unknown audio is never inferred from its directory and remains untouched. Node fields created through **Audio field setup** are not deleted automatically; remove those fields manually if you no longer need them.
