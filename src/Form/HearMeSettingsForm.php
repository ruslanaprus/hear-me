...
class HearMeSettingsForm extends ConfigFormBase {
  ...
  private function buildLimitsAndQuotasSection(array &$form, Config $config): void {
    ...
    $form['max_text_length'] = [
      ...
      '#description' => $this->t('Maximum normalized text length accepted by the TTS endpoint, queue generation, and synthesis service, in characters.'),
  ...
  private function buildQueueAndAudioFieldSection(array &$form, Config $config, mixed $backfillConfirmation, mixed $audioFieldName, array $bundleOptions): void {
    ...
    $form['queue_bundles'] = [
      ...
      '#description'   => $this->t(
        'When a new node of the selected type is created, its configured source text '
        . 'is queued for background TTS synthesis. Leave all unchecked to '
        . 'disable automatic pre-generation entirely. Only published nodes and source fields available to anonymous visitors are eligible. Queue-generated media uses the installed HearMe Audio file field, which stores files publicly by default.'
      ),
      '#options'       => $bundleOptions,
      '#default_value' => $backfillConfirmation['bundles'] ?? $config->get('queue_bundles') ?? [],
    ];

    $form['queue_source_fields'] = $this->buildQueueSourceFieldsElement(
      $bundleOptions,
      $backfillConfirmation['source_fields'] ?? $config->get('queue_source_fields') ?? [],
      $backfillConfirmation['bundles'] ?? $config->get('queue_bundles') ?? [],
    );

    $form['queue_generated_audio_public_warning'] = $this->buildWarning(
      $this->t('Queue-generated audio attached to content is saved as Media/File entities under public://tts/. HearMe queues only published nodes and includes only source fields viewable by anonymous visitors. Review site-specific access rules before enabling a content type.')
    );

    $form['replace_existing_generated_audio'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('Replace existing HearMe-generated audio when content changes'),
      '#description' => $this->t('When enabled, regenerated queue audio replaces media that appears to have been generated by HearMe. This keeps attached audio current after configured source text or language changes.'),
  ...
  private function buildExistingContentBackfillSection(array &$form, mixed $backfillConfirmation): void {
    ...
    $form['existing_content_queue']['public_audio_warning'] = $this->buildWarning(
      $this->t('Backfilled audio is saved under public://tts/ as generated Media/File entities. Only published nodes and source fields viewable by anonymous visitors are eligible.')
    );
  ...
  public function validateForm(array &$form, FormStateInterface $form_state): void {
    ...
    if ($queueBundles) {
      $this->validateQueueSourceFields($queueBundles, $form_state);
  ...
  public function validateQueueExistingContent(array &$form, FormStateInterface $form_state): void {
    ...
    $this->validateQueueSourceFields($bundles, $form_state);
  }
  ...
  public function validateQueueExistingContentConfirmation(array &$form, FormStateInterface $form_state): void {
    ...
    $queueSourceFields = $this->normalizeQueueSourceFieldConfig($form_state->getValue('queue_source_fields') ?? []);
    if (
      $fieldName !== (string) $form_state->getValue('tts_audio_field')
      || !$this->sameBundleSelection($bundles, array_values(array_filter($form_state->getValue('queue_bundles') ?? [])))
      || !$this->sameQueueSourceFieldConfig($confirmation['source_fields'] ?? [], $queueSourceFields)
  ...
  public function submitForm(array &$form, FormStateInterface $form_state): void {
    ...
    $this->configFactory->getEditable('hear_me.settings')
      ...
      ->set('queue_source_fields', $queueSourceFields)
  ...
  public function reviewQueueExistingContentSubmit(array &$form, FormStateInterface $form_state): void {
    ...
    $form_state->set('hear_me_backfill_confirmation', [
      ...
      'source_fields' => $queueSourceFields,
      ...
      'summary' => [
        ...
        '#items' => [
          ...
          $this->t('Publication mode: @mode.', [
            '@mode' => $this->t('published content only'),
          ]),
  ...
  protected function buildQueueSourceFieldsElement(array $bundleOptions, array $sourceConfig, array $queueBundles): array {
    $element = [
      '#type' => 'details',
    ...
    if (!$bundleOptions) {
      $element['empty'] = [
        '#type' => 'item',
        '#markup' => $this->t('No content types are available.'),
      ];
      return $element;
    }

    foreach ($bundleOptions as $bundle => $label) {
      $options = $this->getQueueSourceFieldOptions((string) $bundle);
      $defaults = $this->getQueueSourceFieldDefaults((string) $bundle, $sourceConfig, $options);
      $element[$bundle] = [
        '#type' => 'details',
        '#title' => $this->t('@bundle source text', ['@bundle' => $label]),
        '#open' => in_array($bundle, $queueBundles, TRUE),
        '#states' => [
          'visible' => [
            ':input[name="queue_bundles[' . $bundle . ']"]' => ['checked' => TRUE],
          ],
        ],
      ];
      $element[$bundle]['title'] = [
        '#type' => 'checkbox',
        '#title' => $this->t('Include node title'),
        '#default_value' => $defaults['title'],
      ];
      if ($options) {
        $element[$bundle]['fields'] = [
          '#type' => 'checkboxes',
          '#title' => $this->t('Source fields'),
          '#description' => $this->t('Only supported text and string fields are listed. Paragraphs, entity references, Layout Builder fields, and other complex fields are not supported in this release.'),
          '#options' => $options,
        ...
        $element[$bundle]['empty'] = [
          '#type' => 'item',
          '#markup' => $this->t('No supported source fields are available for this content type.'),
        ];
      }
    }
  ...
  protected function getQueueSourceFieldOptions(string $bundle): array {
    $options = [];
    foreach ($this->entityFieldManager->getFieldDefinitions('node', $bundle) as $fieldName => $definition) {
      if (!$definition instanceof FieldConfigInterface || !in_array($definition->getType(), HearMeNodeAudioQueue::SUPPORTED_SOURCE_FIELD_TYPES, TRUE)) {
        continue;
      }

      $label = (string) $definition->getLabel();
      if ($definition->getType() === 'text_with_summary') {
        $options[$fieldName . ':value'] = $this->t('@label: text', ['@label' => $label]);
        $options[$fieldName . ':summary'] = $this->t('@label: summary', ['@label' => $label]);
      }
      else {
  ...
  protected function getQueueSourceFieldDefaults(string $bundle, array $sourceConfig, array $options): array {
    if (isset($sourceConfig[$bundle]) && is_array($sourceConfig[$bundle])) {
      return [
        'title' => !empty($sourceConfig[$bundle]['title']),
        'fields' => array_values(array_intersect(array_values($sourceConfig[$bundle]['fields'] ?? []), array_keys($options))),
      ];
    }

    return [
      'title' => TRUE,
      'fields' => isset($options['body:value']) ? ['body:value'] : [],
    ];
  }

  protected function normalizeQueueSourceFieldConfig(array $sourceConfig): array {
    $normalized = [];
    foreach (array_keys($this->getNodeBundleOptions()) as $bundle) {
      $options = $this->getQueueSourceFieldOptions((string) $bundle);
      $bundleConfig = $sourceConfig[$bundle] ?? [];
      $fields = [];
      foreach (($bundleConfig['fields'] ?? []) as $token) {
        if (is_string($token) && isset($options[$token])) {
          $fields[] = $token;
        }
      }

      $normalized[$bundle] = [
        'title' => !empty($bundleConfig['title']),
  ...
  protected function validateQueueSourceFields(array $queueBundles, FormStateInterface $form_state): void {
    $sourceConfig = $this->normalizeQueueSourceFieldConfig($form_state->getValue('queue_source_fields') ?? []);
    foreach ($queueBundles as $bundle) {
      $bundleConfig = $sourceConfig[$bundle] ?? ['title' => FALSE, 'fields' => []];
      if (empty($bundleConfig['title']) && empty($bundleConfig['fields'])) {
        $form_state->setErrorByName('queue_source_fields][' . $bundle, $this->t('Select at least one source for the @bundle content type.', [
          '@bundle' => $this->getNodeBundleOptions()[$bundle] ?? $bundle,
        ]));
      }
    }
  }

  protected function sameQueueSourceFieldConfig(array $first, array $second): bool {
  ...
  protected function validateExistingContentQueueSelection(string $fieldName, array $bundles, FormStateInterface $form_state): void {
    $this->validateAudioFieldName($fieldName, 'tts_audio_field', $form_state);
